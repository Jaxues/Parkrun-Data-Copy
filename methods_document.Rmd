...
A lot of the variables were missing data, we overcame this by using data imputation.
We used the mice package in R to perform multiple imputation by chained equations (MICE).
This method fills in missing data by creating multiple complete datasets, analyzing each one separately, and then combining the results.
One issue we came across was the time variable was in a character format (mm:ss) so we converted this to a numeric format (seconds) for the imputation process.
After imputation, we tried to convert it back to the character format, but there was a problem with the lubridate function we used.
So we kept the time variable in seconds for the analysis.
Which is also easier to interpret and analyze statistically.
Originally we also imputed the club variable, But we had not considered that not all runners belong to a club so we decided to keep the missing values as NA, and ignore the club variable in the analysis.

Limitations: The MICE workflow assumes missing values are at least missing at random given the predictors in the model; if time, age, or age grade are absent for slower finishers or certain clubs, the imputed values may still be biased.
Using only a single completed dataset in downstream code also understates the uncertainty introduced by the imputation step.

Future Improvements: Expand the predictor matrix (e.g., include weather or course difficulty proxies), retain and pool results across multiple imputed datasets, and add sensitivity checks that rerun the analysis on an observed-only subset to quantify the impact of imputed values.

To check how well the imputations align with the original data we first source the tidy dataset and MICE model objects, then compare observed and imputed values.

```{r}
source("01_tidy_data.R", echo = FALSE)
```
...
diagnostic_summary <- bind_rows(
  ...
  summarise(
    ...
    .by = c(data_source, variable)
  ) |>
  mutate(data_source = factor(data_source, levels = c("Observed", "Imputed"))) |>
  arrange(variable, data_source) |>
  pivot_wider(names_from = data_source, values_from = c(mean, sd, n_missing)) |> knitr::kable() |> kableExtra::kable_styling(font_size = 8) 

diagnostic_summary
```

### Figure 1.1: Comparison of Observed vs Imputed Values for Age, Age Grade and Time
...
bind_rows(
  ...
  filter(!is.na(time)) |>
  ggplot(aes(x = time, colour = data_source, fill = data_source)) +
  geom_density(alpha = 0.2) +
  theme_minimal() +
  labs(
    title = "Observed vs Imputed Time Distribution",
    x = "Time (seconds)",
    y = "Density",
    colour = "Data source",
    fill = "Data source"
...
### Figure 2: The Distribution of Observed Time Values (blue) Compared to The Distribution of Imputed Time Values (red)

These diagnostics show the summary statistics before and after imputation alongside a density comparison split by observed/imputed values.
Small differences between the paired metrics and overlapping distributions support that the imputation preserved the structure of the original data.

```{r}
time_missing <- is.na(tidy_data$time)

imputed_only <- imputed_data |>
  mutate(source = if_else(time_missing, "Imputed (new)", "Observed (original)")) |>
  filter(source == "Imputed (new)") |>
  select(time, source)

observed_reference <- tidy_data |>
  filter(!is.na(time)) |>
  mutate(source = "Observed (original)") |>
  select(time, source)

bind_rows(observed_reference, imputed_only) |>
  ggplot(aes(x = time, colour = source, fill = source)) +
  geom_density(alpha = 0.3) +
  theme_minimal() +
  labs(
    title = "Distribution of Newly Imputed Times",
    x = "Time (seconds)",
    y = "Density",
    colour = "Series",
    fill = "Series"
...
### Figure 3: The Distribution of The Original Observed Time Values (blue) Compared to The Distribution of New Imputed Time Values (red)

This follow-up view isolates the times that were originally missing and replaced by MICE.
Their density sits within the main observed distribution, indicating that the imputation model is drawing plausible values rather than extreme outliers.
